...
// 核心CNN推理逻辑 (47类 EMNIST)
int simple_cnn_predict(float *input)
{
    static float conv1_out[8 * 26 * 26];
    static float conv2_out[16 * 11 * 11];  // Conv2: 13x13 input, 3x3 kernel, no padding → 11x11 output
    ...
    for (int c = 0; c < 8; c++) {
        for (int y = 0; y < 26; y++) {
            for (int x = 0; x < 26; x++) {
                ...
                for (int dy = 0; dy < 3; dy++) {
                    for (int dx = 0; dx < 3; dx++) {
                        sum += input[(y+dy)*28 + (x+dx)] * conv1_w[c*9 + dy*3 + dx];
                    }
                }
    ...
    for (int c = 0; c < 16; c++) {
        for (int y = 0; y < 11; y++) {
            for (int x = 0; x < 11; x++) {
                float sum = conv2_b[c];
                for (int in_c = 0; in_c < 8; in_c++) {
                    for (int dy = 0; dy < 3; dy++) {
                        for (int dx = 0; dx < 3; dx++) {
                            sum += pool1_out[(in_c*13*13) + (y+dy)*13 + (x+dx)] * conv2_w[c*8*9 + in_c*9 + dy*3 + dx];
                        }
                    }
                }
...
void display_init() {
    ...
    LCD_ShowString(tftlcd_data.width - 40, 25, tftlcd_data.width, tftlcd_data.height, 16, "RST");
    ...
    while(1)
    {
        ...
        if(TP_Scan(0))
        {
            ...
            else
            {
                ...
                {
                    LCD_Fill(0, 0, tftlcd_data.width-1, tftlcd_data.height-16-1, WHITE);
                    FRONT_COLOR = RED;
                    LCD_ShowString(10, 0, tftlcd_data.width, tftlcd_data.height, 16, (u8*)"Char: ");
                    pcnt = 0;
                }
            ...
            if (tcnt == 80)
            {
                if (pcnt > 1)
                {
                    ...
                    if (predict_idx >= 0 && predict_idx < 47) {
                        ...
                        sbuf[1] = '\0';
                        printf("Predicted index: %d, Char: %c\r\n", predict_idx, sbuf[0]);
